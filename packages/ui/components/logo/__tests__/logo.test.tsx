...
describe("<LogoIcon />", () => {
  test("should pass the a11y test", async () => {
    await testA11y(<LogoIcon size="1.6rem" appState="stay-focused" />);
  });
  ...
  test("should match the inline snapshot", () => {
    ...
    const logoIconElement = screen.getByTestId("logo-icon");

    expect(logoIconElement).toMatchInlineSnapshot(`
      <svg
...
describe("<Logo />", () => {
  test("should pass the a11y test", async () => {
    await testA11y(
      <Logo size="1.6rem" appState="stay-focused" appVersion="1.0.0" />
    );
  });
